...
GO
    CREATE PROCEDURE MakeTables AS BEGIN DECLARE @ZoneIDtbl TABLE(zoneID INTEGER);
...
INSERT INTO
    tblDATA (DBNUM, DATACOUNT, Temp, Hum, Lat, Long)
VALUES
    (1, 1, 99.0, 99.0, 0.1, 2.0),
...
CREATE TABLE tblSalesPerson(
    ...
    Name VARCHAR(255),
    MAXDISCOUNT REAL CHECK (
        MAXDISCOUNT >= 0.0
        AND MAXDISCOUNT <= 100.0
    )
);
...
INSERT INTO
    tblZone OUTPUT INSERTED.ID INTO @ZoneIDtbl
VALUES
    ('Nelson Central');

INSERT INTO
    tblZoneBoundaryMarker
VALUES
    (
        (
            SELECT
...
INSERT INTO
    tblContract OUTPUT INSERTED.ID INTO @ContractIDtbl
VALUES
    ('Nelson City Council');
...
GO
    CREATE PROCEDURE SubscribeToDatabox @pSalesID AS INTEGER,
    @pSubFirstName VARCHAR(30),
    @pSubSecondName VARCHAR(30),
    @pSubscriberAddress VARCHAR(255),
    @pSubphone VARCHAR(12),
    @pDiscount REAL,
    @pDataboxID INTEGER AS BEGIN DECLARE @tblID TABLE (SUBID INTEGER);
...
GO
    CREATE PROCEDURE ShowSales @pName VARCHAR(255) AS BEGIN
SELECT
    SubFirstName,
    SubSecondName,
    Discount
FROM
    ...
    JOIN tblSubScription SB ON SP.ID = SB.SalesID
WHERE
    SP.Name = @pName;
